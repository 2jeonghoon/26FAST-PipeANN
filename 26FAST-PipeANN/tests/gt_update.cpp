...
void check_and_gen(char *gt_path, uint64_t tot_npts, uint64_t batch_npts, uint64_t target_topk, char *target_dir,
  ...
  uint64_t max_inserted = tot_npts - batch_npts;
  uint64_t total_ranges = max_inserted / step + 1;
  int success_cnt = 0;
  for (uint64_t st = 0; st <= max_inserted; st += step) {
    bool success = true;
  ...
  LOG(INFO) << "Success rate: " << success_cnt << " / " << total_ranges;
}
